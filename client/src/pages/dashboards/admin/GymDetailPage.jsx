...
import { useGetAdminGymDetailQuery } from '../../../services/dashboardApi.js';
...
const AdminGymDetailPage = () => {
  const { gymId } = useParams();
  const navigate = useNavigate();
  const { data, isLoading, isError, refetch } = useGetAdminGymDetailQuery(gymId);
  const [deleteGym, { isLoading: isDeleting }] = useDeleteGymMutation();
  const [notice, setNotice] = useState(null);

  const gym = data?.data;

  const handleBack = () => navigate('/dashboard/admin/gyms');
    ...
    <div className="dashboard-grid dashboard-grid--stacked ud-page">
      ...
      <DashboardSection title="Gym Information">
        <div className="ud-card">
          ...
          {gym.pricing && (
            <p><strong>Pricing:</strong> {currency(gym.pricing.monthlyPrice)}/mo (MRP: {currency(gym.pricing.monthlyMrp)})</p>
          )}

          {gym.schedule && (
            <p><strong>Schedule:</strong> {gym.schedule.openTime} – {gym.schedule.closeTime} · {(gym.schedule.workingDays ?? []).join(', ')}</p>
          )}
              ...
              <tbody>
                {allMembers.map((m) => (
                  <tr key={m.id}>
                    ...
                    <td>{m.plan}</td>
                    <td>
                      <span className={`status-pill ${m.status === 'active' ? 'status-pill--success' : 'status-pill--warning'}`}>
              ...
              <tbody>
                {gym.assignments.map((a) => (
                  <tr key={a.id}>
                    ...
                    <td>
                      <span className={`status-pill ${a.status === 'active' ? 'status-pill--success' : 'status-pill--warning'}`}>
              ...
              <tbody>
                {gym.subscriptions.map((s) => (
                  <tr key={s.id}>
                    ...
                    <td>
                      <span className={`status-pill ${s.status === 'active' ? 'status-pill--success' : 'status-pill--warning'}`}>
...
export default AdminGymDetailPage;
