...
const GymOwnersPage = () => {
  ...
  const filteredOwners = useMemo(() => {
    ...
    return owners.filter((owner) => {
      ...
      return [owner.name, owner.email].filter(Boolean).some((value) => value.toLowerCase().includes(query));
    });
  ...
  const handleToggleStatus = async (owner) => {
    ...
    try {
      ...
      setErrorNotice(error?.data?.message ?? 'Unable to update gym owner status.');
    }
  };
  ...
  const handleDelete = async (owner) => {
    ...
    try {
      ...
      setErrorNotice(error?.data?.message ?? 'Unable to delete gym owner.');
    }
  };
                ...
                <tr key={owner._id}>
                  <td>
                    <div
                      ...
                      role="button"
                      tabIndex={0}
                      onKeyDown={(event) => event.key === 'Enter' && navigate(`/dashboard/manager/users/${owner._id}`)}
                    >
                      ...
                      <div>
                        ...
                        {owner.profile?.headline ? <div><small>{owner.profile.headline}</small></div> : null}
                      </div>
                    </div>
                  </td>
                  <td>{owner.email}</td>
