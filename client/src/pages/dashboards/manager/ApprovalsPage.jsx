...
const ApprovalsPage = () => {
  ...
  const filteredPending = useMemo(() => {
    ...
    return pending.filter((user) => user.role === roleTab);
  }, [pending, roleTab]);
  ...
  const handleApprove = async (user) => {
    ...
    try {
      ...
      setErrorNotice(error?.data?.message ?? 'Unable to approve this user.');
    }
  };
  ...
  const handleReject = async (user) => {
    ...
    try {
      ...
      setErrorNotice(error?.data?.message ?? 'Unable to reject this user.');
    }
  };
                  ...
                  <td>
                    <div className="dashboard-table__user">
                      ...
                      <div>
                        ...
                        {user.profile?.headline ? <div><small>{user.profile.headline}</small></div> : null}
                      </div>
                    </div>
                  </td>
                  <td>{user.email}</td>
                  <td>
                    <span className="status-pill status-pill--warning">{formatStatus(user.role)}</span>
                  </td>
