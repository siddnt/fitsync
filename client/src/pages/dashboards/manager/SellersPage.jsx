...
const SellersPage = () => {
  ...
  const filteredSellers = useMemo(() => {
    ...
    return sellers.filter((seller) => {
      ...
      return [seller.name, seller.email].filter(Boolean).some((value) => value.toLowerCase().includes(query));
    });
  ...
  const handleToggleStatus = async (seller) => {
    ...
    try {
      ...
      setErrorNotice(error?.data?.message ?? 'Unable to update seller status.');
    }
  };
  ...
  const handleDelete = async (seller) => {
    ...
    try {
      ...
      setErrorNotice(error?.data?.message ?? 'Unable to delete seller.');
    }
  };
                ...
                <tr key={seller._id}>
                  <td>
                    <div
                      ...
                      role="button"
                      tabIndex={0}
                      onKeyDown={(event) => event.key === 'Enter' && navigate(`/dashboard/manager/users/${seller._id}`)}
                    >
                      ...
                      <div>
                        ...
                        {seller.profile?.headline ? <div><small>{seller.profile.headline}</small></div> : null}
                      </div>
                    </div>
                  </td>
                  <td>{seller.email}</td>
